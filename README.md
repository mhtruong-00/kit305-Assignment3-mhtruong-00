...
The overall application design, data model, navigation flow, validation rules, business logic (quote / labour / discount maths, compatibility constraints), Storyboard layout, debugging, and testing in the simulator were all done by me. My main references were the KIT305 tutorial codebase and my own Assignment 2 Android implementation.

**GitHub Copilot** (inline suggestions in Xcode + occasional ChatGPT questions) was used as an assistant for the narrow tasks listed below. Every suggestion was read, edited, and tested before being kept.

| Where I used AI | What I asked for | What I still did myself |
|---|---|---|
| `FirestoreService` | Boilerplate for snapshot listeners and `Codable` decoding patterns | Designed the flat-collection schema to match my Android A2, wrote the cascading-delete logic, chose which fields to denormalise. |
| `PhotoPickerCoordinator` | Reminder of the `PHPickerViewController` + `UIImagePickerController` API surface | Wired the action-sheet UX, camera-availability fallback, delegate plumbing, and the `Info.plist` privacy keys. |
| `ImageStore` | Suggestion for a JPEG quality-stepping loop to keep size under a budget | Picked the 250 KB target (Firestore doc-size constraint) and decided to store as base64 inside the document instead of using Firebase Storage. |
| `CSVExporter` | Reminder of `UIActivityViewController` setup and CSV-escaping rules | Designed the multi-section row layout so the iOS export matches the Android A2 export. |
| Small Swift snippets | Syntax for things like `URLSession` JSON decoding, `UIRefreshControl`, `UIContextualAction` swipe actions | Integrated into the right controllers, handled error paths, tested in the simulator. |
| README & commit messages | Proofreading and table formatting | Wrote the content; AI only tidied wording. |

I did **not** use AI to generate the assignment as a whole, to design the Firestore schema, to write the compatibility rules, or to make UX decisions. Where Copilot's output was wrong (for example an integer-division bug in the panel-width check) I caught and fixed it manually. No third-party libraries other than Firebase (allowed) were added.
