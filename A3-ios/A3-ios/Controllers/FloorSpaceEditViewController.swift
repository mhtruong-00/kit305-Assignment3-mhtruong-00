...
class FloorSpaceEditViewController: UIViewController {
    ...
    @objc private func saveTapped() {
        ...
        guard let lengthText = lengthField.text, let length = Double(lengthText), length > 0 else {
            showAlert(title: "Validation Error", message: "Length must be a positive number.")
            return
        }
